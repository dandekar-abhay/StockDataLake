...
config.read('date_config.txt')
DATE = config.get("date", "sdate")
os.chdir(f'../../bin/Stock_Data_Extraction/')
# print(str(DATE))
prefs = {"download.default_directory": f"{abs_path}/../../data/data_{DATE}/Comp_data/"}
chromeOptions = webdriver.ChromeOptions()
chromeOptions.add_experimental_option("prefs", prefs)

chrome = webdriver.Chrome('/usr/bin/chromedriver',options=chromeOptions)
...
login.click()
index = 0
file1 = open(f'./2_error_fetch_{DATE}.txt', 'a')
file1.write(f'----------------------------------------------------------------\n')
file1.write(f'-----------FOLLOWING DATA WAS GATHERED ON: {datetime.now()}-----------------\n')
file1.write(f'----------------------------------------------------------------\n')

file2 = open(f'./2_files_gathered_{DATE}.txt', 'a')
file2.write(f'----------------------------------------------------------------\n')
file2.write(f'-----------FOLLOWING DATA WAS GATHERED ON: {datetime.now()}-----------------\n')
file2.write(f'----------------------------------------------------------------\n')

with open('./2_scriptstock.csv', 'rt') as f:
    data = csv.reader(f)
    for row in data:
        print(row)
